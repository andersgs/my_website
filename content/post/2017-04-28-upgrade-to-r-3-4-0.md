...
Upgrading `R` can be a pain. Mostly because you may find yourself without any of
...
The `.Renviron` file is read by `R` everytime you start `R`. It reads environmental
...
(for instance, the greate [`ggtree`](https://github.com/GuangchuangYu/ggtree)),
...
The `ask= F` option will refain from asking you questions. The `checkBuilt = T`
